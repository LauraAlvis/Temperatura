...
def main():
    try:
        ...
        with st.form(key="conversion_form"):
            if mode == "Celsius → Fahrenheit":
                ...
                if submit:
                    ...
                    f_from_c = c_to_f(c)
                    c_from_f = f_to_c(f)
                    st.write(f"Desde {c:.{decimals}f} °C → {f_from_c:.{decimals}f} °F")
                    st.write(f"Desde {f:.{decimals}f} °F → {c_from_f:.{decimals}f} °C")
        ...
        with st.expander("Información técnica / Debug"):
            st.write(f"Python: `{sys.version.split()[0]}`")
            try:
                st.write(f"Streamlit: `{st.__version__}`")
            except Exception:
                st.write("No se pudo leer la versión de Streamlit")
            st.write("Si la app aparece en blanco al desplegar, revisa los logs del servicio (Streamlit Cloud → Manage app → Logs).")
    except Exception as e:
        # Capturamos cualquier excepción y la mostramos en la UI para evitar pantalla en blanco
        st.error("Se produjo un error al ejecutar la aplicación.")
        st.text("Traceback (información técnica):")
